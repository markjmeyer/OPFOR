function [beta,gamma,alpha]=UpdateBetaNoOrthog(beta,Vbeta,PI,T,Wv,model,Bspecs,MCMCspecs)
...
for i=1:p  %#zhu#% compute beta_i conditional on beta_{-i} here, it is the correct as we derived, for each j,k, beta_{ijk} updated alternatively.
    ...
    %%
    [beta(i,:),gamma(i,:),alpha(i,:)]=UpdateBeta(Betansi(i,:),Vbeta(i,:),PI(i,:),T(i,:),MCMCspecs);
end % i loop
